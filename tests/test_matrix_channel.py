...
import gssbot.channels.matrix as matrix_module
from gssbot.bus.events import OutboundMessage
from gssbot.bus.queue import MessageBus
from gssbot.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from gssbot.config.schema import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("gssbot.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "gssbot.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("gssbot.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "gssbot.channels.matrix.asyncio.create_task", _fake_create_task
    )
